...
<div class="w">
    ...
    <div class="mainbody">
        ...
        <div id="left" class="box">
            <div class="diku" v-show=true>
                <div class="dikutitle">黑库列表</div>
                <div class="dikutitle">当前已注册:{{ black_numbers }} 人</div>
                <div class="dikutitle">系统状态: 自动注册中</div>
                <div class="dikutitle">今日已自动注册: xx 人</div>
                <div class="dikutitle">今日自动检测准确率: xx%(top5)</div>
            </div>
            <div class="">最近注册用户列表</div>
            <ul>
                <li v-show="true" class="lilis" :class="activeClass == index ? 'active' : ''" v-for="(item, index) in blackbase_names" :key="index" @click="tapcross(index)">
                    {{ item }}
                </li>
            </ul>
        </div>
        ...
        <div id="middle">
            ...
            <div class="mid_mid">
                <el-button id="startbtn" ref="startbtn" @click="open" :disabled="startbtnClickable">开始识别</el-button>

                <el-button id="startbtn" ref="startbtn" @click="register" :disabled="startbtnClickable">开始注册</el-button>
            </div>
            <!-- 分析结果  -->
            <div id="mid_bottom" class="box">
                <img id="loadImg" src="../assets/img/inLoad.gif" v-show="showLoading" />
                <div class="result_img" v-show="showResult">
                    <img src="../assets/img/dui.png" v-show="inBase" />
                    <img src="../assets/img/cuo.png" v-show="!inBase" />
                </div>
                <div class="end_result">
                    <p class="score" v-show="showResult" :class="[inBase ? same : unsame]">
                        {{ inBase ? "在黑库中！" : "不在黑库中！" }}
                        最高评分为：{{ bestScore }}
                    </p>
                </div>
        ...
        <div id="right" class="box">
            <div class="text_title">
                <label id="changebtn" type="primary" @click="changeTxt()">换一篇</label>
            </div>

            <li>
                <p>{{ textArr[this.currentTextIndex].name }}</p>
            </li>
        </div>
    </div>
    <div class="footer">
        <div class="content">
            ...
            <a href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=2021036408 ">苏ICP备2021036408号</a>
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        changeTxt() {
            ...
            this.currentTextIndex = currentIndex;
        },

        // 底库接口
        basewav() {
            this.$axios.get("/namelist").then((res) => {
                if (res.code == 2000) {
                    if (res.status) {
                        let names = res.names_10.reverse();
                        this.blackbase_names = names;
                        this.black_numbers = res.numbers;
                    } else {
                        console.log(res);
                    }
                }
        ...
        startRegister(e) {
            ...
            fd.append("wav_file", this.recordedData);
            fd.append("spkid", e);
            that.$axios
                .post("/register/file", fd, {
                    headers: {
                    ...
                    if (res.code == 2000) {
                        if (res.status == "success") {
                            ...
                            this.$notify({
                                ...
                                message:  "原始长度:"+res.raw_length+"<br/>"+"有效长度:"+res.vad_length+"<br/>"+"状态:"+res.status,
                                type: 'success'
                            });
                ...
                .then((res) => {
                    if (res.code == 2000) {
                        if (res.status == "success") {
                            ...
                            // 是否在黑库中
                            that.inBase = res.scores.inbase;

                            // 最高评分
                            that.bestScore = res.scores.best_score;

                            // 弹窗显示结果
                            
                            this.$notify({
                                dangerouslyUseHTMLString: true,
                                message: "原始长度:"+res.raw_length+"<br/>"+"有效长度:"+res.vad_length+"<br/>"+res.top_list,
                                type: 'success'
                            });
        ...
        tapcross(i) {
            // this.startAnalysisInsert(this.blackbase_names[i]);
            // this.activeClass = i;
            console.log(this.blackbase_names[i]);
        },

        //websocket相关
        initWebSocket(){ 
        //初始化 
        this.websocket = new WebSocket('ws://iint.icu:8170/echo');  
        ...
        websocketonopen() { 
            //发送
            this.websocket.send('websocket from vue'); 
        ...
        websocketonmessage(e){ 
            ...
            this.black_numbers=resData.number;
            this.blackbase_names = resData.names_10.reverse();
            // alert(e.data);
